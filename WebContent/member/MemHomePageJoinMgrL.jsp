...
// 강제탈퇴
function fn_compulsionSecede(value) {
	var fm = document.fmParam;
	
	if (confirm("선택된 회원을 강제탈퇴 하시겠습니까?")) {
		fm.action = "MemAction.do?cmd=MemHomePageSecedeU";
...
<form name="fmParam" method="post"  action="MemAction.do?cmd=MemHomePageJoinMgrL">
							...
							<tr>
								<td> 
									<select name="scSLCT_GUBUN">
										<option value="ID" <%=KJFUtil.print(pm.getScSLCT_GUBUN()).equals("ID")?"selected":""%>>ID</option>
...
<table width="100%" border="0"  cellspacing="0" cellpadding="0">
  	<tr>
		...
		<td width="10%" class="listTable_header_section"><strong>강제탈퇴</strong></td>
	</tr>
	...
	<tr> 
		...
		<td class="listTable_td"><%=rEntity.getValue(i, "USER_ID")%></td>
		<td class="listTable_td"><%=rEntity.getValue(i, "USER_NAME")%></td>		
		...
		<td class="listTable_td" align="center"><a href="javascript:fn_compulsionSecede('<%=rEntity.getValue(i, "USER_ID")%>');">
