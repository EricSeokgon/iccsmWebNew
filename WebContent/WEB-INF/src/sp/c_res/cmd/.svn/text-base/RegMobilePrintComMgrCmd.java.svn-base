...
public class RegMobilePrintComMgrCmd implements KJFCommand{
	
	private String next;	
    private DatasetList  dl = new DatasetList();
    private VariableList vl = new VariableList();
    
    public RegMobilePrintComMgrCmd() {
    }

    public RegMobilePrintComMgrCmd(String next_url) {
    	next = next_url;
    }
    ...
    public String execute(HttpServletRequest request, ActionForm form) throws Exception {
        ...
        sQuery.append("        '-' AS INSTAL_NAME, '-' AS USE, '-' AS REMARK, SUM(QTT) AS QTT    \n");
