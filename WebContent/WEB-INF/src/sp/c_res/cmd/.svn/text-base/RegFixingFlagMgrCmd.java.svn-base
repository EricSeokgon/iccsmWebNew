...
public class RegFixingFlagMgrCmd implements KJFCommand{
	
	private String next;	
    private DatasetList  dl = new DatasetList();
    private VariableList vl = new VariableList();
     
    public RegFixingFlagMgrCmd() {
    }

    public RegFixingFlagMgrCmd(String next_url) {
    	next = next_url;
    }
    ...
    public String execute(HttpServletRequest request, ActionForm form) throws Exception {
        ...
        sQuery.append(" SELECT '0' AS CHECKER ,FF.* ");
        sQuery.append(" FROM PT_C_RES_FIXING_FLAG FF");
