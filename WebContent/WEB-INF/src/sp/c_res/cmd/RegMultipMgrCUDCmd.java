...
import sp.dao.PT_C_RES_MULTIPLEXINGDAO;
import sp.dao.PT_C_RES_MULTIPLEXINGEntity;
...
public class RegMultipMgrCUDCmd implements KJFCommand {
	
    private String next;
    
    private PT_C_RES_MULTIPLEXINGDAO    dao = new PT_C_RES_MULTIPLEXINGDAO();
    private PT_C_RES_MULTIPLEXINGEntity Eent = new PT_C_RES_MULTIPLEXINGEntity();
   
    private VariableList vl = new VariableList();
    
    public RegMultipMgrCUDCmd() {
    }


    public RegMultipMgrCUDCmd(String next_url) {
    	next = next_url;
    }
    ...
    public String execute(HttpServletRequest request, ActionForm form) throws Exception {   	    	
    	...
    	Dataset ds = pReq.getDatasetList().getDataset("dsPT_C_RES_MULTIPLEXING");
    ...
    private void insertExe(Dataset ds, int arg_row)throws Exception{
    	...
    	if(ds.getId().equalsIgnoreCase("dsPT_C_RES_MULTIPLEXING"))
    	{
    		...
    		String SEQ = KJFDBUtil.getMaxID("SEQ","PT_C_RES_MULTIPLEXING");
    ...
    private void updateExe(Dataset ds, int arg_row)throws Exception{
    	...
    	if(ds.getId().equalsIgnoreCase("dsPT_C_RES_MULTIPLEXING"))
    	{
	    	...
	    	Eent.setDsOnlyString(ds, arg_row);//     	
	    	String[] fields={"SEQ", "WRT_ID", "SIDO_CODE","SIGUNGU_CODE", "INS_DT" };   	
	    	/******특정한 필드를 제외한 수정 ******/
    ...
    private void deleteExe(Dataset ds, int arg_row)throws Exception{
    	...
    	if(ds.getId().equalsIgnoreCase("dsPT_C_RES_MULTIPLEXING"))
    	{
			...
			deleteQuery.append(" DELETE FROM PT_C_RES_MULTIPLEXING  WHERE SIDO_CODE = '"+SIDO_CODE+"' AND SEQ = '"+SEQ+"' ");
