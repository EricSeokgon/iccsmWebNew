...
public class RegMgrVCmd implements KJFCommand {
	...
	private void resultLoadDetail(HttpServletRequest request) throws Exception {
        ...
        String whereSQL = 
        	...
        	" 	AND TMP_WRT_NUM = (SELECT TMP_WRT_NUM FROM PT_R_COMPANY_MASTER WHERE COI_WRT_NUM = '"+p_coi_wrt_num+"' " +
        			"				AND  (C_COM_DT IS NULL OR TRANS_UNION_PROC IS NULL OR TRANS_UNION_PARENT IS NULL)  )  \n";
	...
	private void disposeLoadDetail(HttpServletRequest request) throws Exception {
        ...
        String selectSQL =
			" SELECT      \n"+ 
			" 	WRT_NUM, DISPO_DT, WRT_DT, DISPO_CONT_NM, DISPO_CAUSE, PUNISHMENT_AGENCY, BUSISUSP_START_DT, BUSISUSP_END_DT    \n";	
	...
	private void technicianLoadDetail(HttpServletRequest request) throws Exception {
        ...
        String whereSQL = 
        	" 	WHERE 1=1 \n"+
        	" 	AND TMP_WRT_NUM = (SELECT TMP_WRT_NUM FROM PT_R_COMPANY_MASTER WHERE COI_WRT_NUM = '"+p_coi_wrt_num+"'" +
        			"	AND  (C_COM_DT IS NULL OR TRANS_UNION_PROC IS NULL OR TRANS_UNION_PARENT IS NULL) ) AND RET_YMD IS NULL \n";
