...
public class NameChgListMgrCmd implements KJFCommand {
	
    private String next;	
    private DatasetList  dl = new DatasetList();
    private VariableList vl = new VariableList();
    
    public NameChgListMgrCmd () {
    }

    public  NameChgListMgrCmd (String next_url) {
    	next = next_url;
    }
...
//        sQuery.append(" FROM PT_R_COMPANY_MASTER PT_M ");
     /*   
        sQuery.append(" SELECT PT_H.COI_WRT_NUM, PT_H.CHG_DT,PT_H.CHGBRE_SEQ,PT_R.NAME AS NAME, PT_H.NAME AS RE_NAME, PT_H.TMP_WRT_NUM, PT_C.CODE_NAME, PT_H.WRT_DT  ");
        sQuery.append(" FROM PT_R_BASIC_CHANGE_HISTORY PT_H LEFT JOIN PT_R_COMPANY_MASTER PT_R ");
        sQuery.append(" ON PT_H.TMP_WRT_NUM = PT_R.TMP_WRT_NUM  LEFT JOIN  (SELECT CODE, CODE_NAME FROM   ");
        sQuery.append(" PT_COM_CODE WHERE P_CODE = 'REGDIV') PT_C ON PT_H.STA_CHG_CLASS_CODE = PT_C.CODE   ");
        sWhere.append(" WHERE 1 = 1 ");
        sWhere.append(" AND PT_H.TMP_WRT_NUM = '" + scTMP_WRT_NUM + "'");
        sWhere.append(" AND PT_C.CODE='R00010' ");
    	sWhere.append( " AND (PT_H.CHG_DT BETWEEN '" + scDATE1 + "' " );
		sWhere.append( " AND '" + scDATE2 + "') " );
		*/
        sQuery.append(" select a.COI_WRT_NUM, b.name, a.WRT_CHG_DT, a.CHG_STA_DT, FNGetPrevName(a.COI_WRT_NUM, a.WRT_CHG_DT ) prev_name, a.name as RE_NAME");
        sQuery.append(" from PT_TMP_NAME_CHANGE a,  PT_R_company_master b");
        sQuery.append(" where 1=1 ");
        sQuery.append(" and a.seq in(  ");
        sQuery.append(" select max(seq) seq from PT_TMP_NAME_CHANGE ");
