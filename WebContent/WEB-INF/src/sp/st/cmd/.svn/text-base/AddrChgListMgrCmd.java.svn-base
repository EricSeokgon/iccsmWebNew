...
public class AddrChgListMgrCmd implements KJFCommand {
	
    private String next;	
    private DatasetList  dl = new DatasetList();
    private VariableList vl = new VariableList();
    
    public AddrChgListMgrCmd () {
    }

    public  AddrChgListMgrCmd (String next_url) {
    	next = next_url;
    }
...
//        sQuery.append(" FROM PT_R_COMPANY_MASTER PT_M ");
        /*
        sQuery.append(" SELECT PT_H.COI_WRT_NUM, PT_H.CHG_DT, PT_H.CHGBRE_SEQ, PT_R.NAME, PT_H.TMP_WRT_NUM,PT_C.CODE_NAME, PT_H.WRT_DT,RTRIM(PT_H.ADDR_ADDR)||''||PT_H.ADDR_DETAIL_ADDR AS ADDR_ADDR ");
        ...
        sWhere.append(" AND PT_C.CODE='R00007' ");
        sWhere.append(" AND PT_C.CODE is not null ");
    	sWhere.append( " AND (PT_H.CHG_DT BETWEEN '" + scDATE1 + "' " );
		sWhere.append( " AND '" + scDATE2 + "') " );
		*/
        sQuery.append(" select a.COI_WRT_NUM, b.name, a.WRT_CHG_DT, a.CHG_STA_DT, FNGetPrevAddr(a.COI_WRT_NUM, a.WRT_CHG_DT ) prev_Addr , a.addr_addr||a.ADDR_DETAIL_ADDR addr");
        sQuery.append(" from PT_TMP_ADDR_CHANGE a, PT_R_company_master b ");
        sQuery.append(" where 1=1 ");
        sQuery.append(" and a.seq in( ");
        sQuery.append(" select max(seq) seq from PT_TMP_ADDR_CHANGE");
