/**
 * 파일명   : BackupLinkMgrCmd.java 
 * 설명     : 리스트 cmd  
 * 이력사항
 * CH00     :2006/08/15 전원배 최초작성 
 */
...
import sp.util.transdb.BackupAgent;
import sp.util.transdb.BackupTask;

public class BackupLinkMgrCmd implements KJFCommand {


	private String next;
	
    public BackupLinkMgrCmd() {
    }
    
    public BackupLinkMgrCmd(String next_url) {
    	next = next_url; 
    }
    ...
    public String runExe(HttpServletRequest request) throws Exception {
        ...
        if(BackupAgent.bscheduler==null){

        	bt ="bt_start";
        	if(mode.equals("run")) {
        		System.out.print("==============BackupAgent run===============");
        		BackupAgent.start();
        		bt ="bt_stop";
        	}
        	
        }else{
        	System.out.println("===========BackupAgent not null =========");
        	bt ="bt_stop";
        	if(mode.equals("run")) {
            	
        		BackupAgent.bscheduler.shutdown();
            	System.out.println("KicaAgent.isShutdown(): "+BackupAgent.bscheduler.isShutdown());
            	BackupAgent.bscheduler=null;
            	bt ="bt_start";
        	}
