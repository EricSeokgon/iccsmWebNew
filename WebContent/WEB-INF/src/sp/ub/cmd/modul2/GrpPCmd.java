...
public class GrpPCmd {
    ...
    private String send_plan_file_name =""; // 착공전설계 결과 통보서
    private String send_plan_file_name2 =""; // 착공전설계 결과 보고서
    ...
    private String INSP_SPOT_DETAILADDR = "";	// 시공현장 세부주소
    ...
    private String WORK_ITEM_ETC2 = "";	// 방송공동수신 설비
    private String AREA = "";			// 면적
    private String NUM_FL = "";			// 층수
    private String NUM_FL2 = "";			// 층수
	...
	private String PLANER_NAME = "";	// [착공전] 시공업체명
	private String PLANER_NM = "";		// [착공전] 설계자명 
	private String STRU_COMMIT_NUM =""; // [착공전] 건축허가번호 
	private String STRU_ADDR_ADDR = ""; // [착공전] 건축물 소재지 (주소)
	private String STRU_ADDR_DETAILADDR = ""; // [착공전] 건축물 상세소재지 (주소2)

	private String SUV_APPL = "";		// [착공전] 검토 구분(적합/부적합/감리대상/비대상)
	private String INSP_OPIN ="";		// [착공전] 검토 의견
	private String PRE_ITEM ="";		// [착공전] 보완 사항
	
	private String ApplGugun="E"; 		//전자결재 시스템 유형
	
	private String PLAN_ENT_CHG_YN ="";	//착공전설계업체명 변경유무
	private String PLAN_ENT_CHG_YN_CONT ="";//변경된 착공전설계업체명
    ...
    public void Grp_Exe(HttpServletRequest request, VariableList vl, ActionForm form) throws Exception {
        ...
        ApplGugun		= KJFUtil.print(vl.getValueAsString("scAPPL_GUBUN"),"E");//전자결재시스템 유형
...
//    	String dirPath = "D:\\work\\attache\\iccsm\\usebefore\\_ebms_dir\\";
    ...
    private String setExchangePlanFile(UbParam pm)throws Exception{
    	...
    	if ("".equals(ORPE_NM)){
    		...
    		send_plan_file_name = "설계도 확인 결과 통보서";
    	}
    	else {
    	...
    	if ("ggyc".equals(pm.getScSIGUNGU_CODE())){
    		xmlData.append("        1. 허가민원과-"+pm.getScCIV_RECV_NUM()+"("+ RECV_DT +") 호와 관련입니다.\n");
    	}
    	else if ("gbus".equals(pm.getScSIGUNGU_CODE())){
    		xmlData.append("        1. 민원과-"+pm.getScCIV_RECV_NUM()+"("+ RECV_DT +") 호와 관련입니다.\n");
    	}
    	else if ("gbcd".equals(pm.getScSIGUNGU_CODE())){
    		xmlData.append("        1. 도시과-"+pm.getScCIV_RECV_NUM()+"("+ RECV_DT +") 호와 관련입니다.\n");
    	}
    	else if ("gguw".equals(pm.getScSIGUNGU_CODE())){
    		xmlData.append("        1. 건축디자인과-"+pm.getScCIV_RECV_NUM()+"("+ RECV_DT +") 호와 관련입니다.\n");
    	}
     	else if ("cbji".equals(pm.getScSIGUNGU_CODE())){
    		xmlData.append("        1. 지역개발건축과-"+pm.getScCIV_RECV_NUM()+"("+ RECV_DT +") 호와 관련입니다.\n");
    	}
    	else if ("jnwd".equals(pm.getScSIGUNGU_CODE())){
    		xmlData.append("        1. 귀사의 무궁한 발전을 기원합니다.\n");
    	}
    	else {
    	...
    	if ("jnwd".equals(pm.getScSIGUNGU_CODE())){
    		xmlData.append("        2. " + STRU_ADDR_ADDR + " 건축허가에 따른 정보통신공사 설계도확인과 관련하여 정보통신공사업법 제36조의 규정에 의거 구내 정보통신시설 적정 여부 등에 대한 심사의견을 붙임과 같이 통보합니다.\n\n");
    	}
    	else{
    	    xmlData.append("        2. 정보통신공사업법 제 36조에 의거 정보통신공사 설계도확인과 관련하여 구내 정보통신시설 적정 여부 등 우리과 소관사항에 대한 심사의견을 붙임과 같이 통보합니다.\n\n");
    	}
    	//xmlData.append("           여부 등 우리과 소관사항에 대한 심사의견을 붙임과 같이 통보합니다.\n");
    	xmlData.append("          가. 건 축 주 : " + ORPE_NM + " \n");
    	...
    	if ("jnwd".equals(pm.getScSIGUNGU_CODE())){
        	xmlData.append("          나. 위       치 : " + STRU_ADDR_ADDR +" ("+NUM_FL+"층, "+"연면적 : "+AREA+"㎡ ) \n"); 
        	xmlData.append("          다. 주 용 도 : "+ OFFICE_TYPE+"\n");
    	}
    	else{
    	xmlData.append("          나. 위  치 : " + STRU_ADDR_ADDR +" ("+AREA+"㎡ /"+ NUM_FL2 +") \n"); 
...
//	    	}
    	...
    	} else if ("비대상".equals(SUV_APPL)){
    		xmlData.append("          라. 심사대상 : " + WORK_ITEM+ WORK_ITEM_ETC + "\n");
    		xmlData.append("          마. 심사의견 : " + SUV_APPL + ".  끝.\n");
    	} else {
        	xmlData.append("          라. 심사대상 : " + WORK_ITEM+ WORK_ITEM_ETC + "\n");
        	xmlData.append("          마. 심사의견 : " + SUV_APPL + "\n");
    	}
    	...
    	if (!"비대상".equals(SUV_APPL)){
    		if (!"N".equals(ApplGugun)){
    			...
    			xmlData.append("\n  붙임  설계도 확인 결과 통보서 1부.  끝.\n");
    			}else{
    				xmlData.append("\n  붙 임 : 설계도 확인 결과 통보서 1부.  끝.\n");
    			}
    		}
    	}
...
//    	String dirPath = "D:\\work\\attache\\iccsm\\usebefore\\_ebms_dir\\";
    	...
    	if(!makeEbmsPlanFileInvest(pm, filePath)){
    		KJFLog.log("파일 생성 실패4");
    	}else{
    		KJFLog.log("파일 생성 완료4");
    		if(!sendEbmsPlanFileInvest(pm, filePath, destFolder)){
    			KJFLog.log("파일 전송 실패4");
        	}else{
        		KJFLog.log("파일 전송 완료4");
        	}
    	}
    	
    	while(true){
            try{
            	KJFLog.log("파일 전송 eof.inf4");
            	Thread.sleep(1000);            	
            }catch(Exception e){
                System.out.println(e.toString());
            }
    ...
    public boolean makeEbmsPlanFileInvest(ActionForm form, String filePath)throws Exception{
    	...
    	//String exchangeData = setExchangePlanFile(pm);	//exchange.xml 파일 내용
    	//String ozResultData = setAttachPlanFile(pm);	//attach_UBInvestResult.html 파일 내용
    	String headerData 	= setHeaderFile_other(pm,"_Invest");	//header.inf 파일 내용
    	String exchangeData = setExchangePlanFileInvest(pm,"_Invest");	//exchange.xml 파일 내용
    	String ozResultData = setAttachPlanFileInvest(pm);	//attach_UBInvestResult.html 파일 내용
    ...
    private String setExchangePlanFileInvest(UbParam pm, String num)throws Exception{
    	...
    	if ("".equals(ORPE_NM)){
    		//send_plan_file_name = "설계도 확인 결과 통보서_("+SUV_APPL.trim()+")"+pm.getScCIV_RECV_NUM();
    		send_plan_file_name2 = "설계도 검토 결과 보고서_("+SUV_APPL.trim()+")"+REG_NUM;
    	} else {
    		//send_plan_file_name = "설계도 확인 결과 통보서_("+ORPE_NM+"-"+SUV_APPL.trim()+")_"+pm.getScCIV_RECV_NUM();
    		send_plan_file_name2 = "설계도 검토 결과 보고서_("+ORPE_NM+"-"+SUV_APPL.trim()+")_"+REG_NUM;
    	}
    	    	*/
    	
    	if ("".equals(ORPE_NM)){
    		//send_plan_file_name = "설계도 확인 결과 통보서_("+SUV_APPL.trim()+")"+pm.getScCIV_RECV_NUM();
    		send_plan_file_name2 = "설계도검토 결과 보고서 1부";
    	} else {
    		//send_plan_file_name = "설계도 확인 결과 통보서_("+ORPE_NM+"-"+SUV_APPL.trim()+")_"+pm.getScCIV_RECV_NUM();
    		send_plan_file_name2 = "설계도검토 결과 보고서 1부";
    	}
    	...
    	xmlData.append("				<USERID>" 	+ pm.getRecvUserID()	+ "</USERID>\n");
    	xmlData.append("			</RECEIVER>\n");
    	xmlData.append("			<TITLE>정보통신공사 착공전 설계도 검토 결과("+SUV_APPL+")보고</TITLE>\n");
    	xmlData.append("			<CREATED_DATE>"	+ pm.getSendDate()		+ "</CREATED_DATE>\n");
    	...
    	xmlData.append("	</HEADER>\n");
    	xmlData.append("	<BODY><![CDATA[");
        xmlData.append("        정보통신공사 착공전 설계도 확인 신청건(접수번호-"+pm.getScCIV_RECV_NUM()+", " + RECV_DT + ")에 대하여");
        xmlData.append(" 정보통신공사업법 제36조에 의하여 검토한 결과를 붙임과 같이 보고합니다.\n\n\n");
        xmlData.append("        ▣ 검토내역\n");
    	
    	 	    	
    	xmlData.append("          1. 신청인(건축주) : " + ORPE_NM + " \n");
    	...
    	xmlData.append("          2. 공사의 종류 : " + WORK_ITEM+ ", " + WORK_ITEM_ETC2 + "\n"); 
    	
    	xmlData.append("          3. 건축장소 : "+ STRU_ADDR_ADDR + "  " + STRU_ADDR_DETAILADDR +" \n");
    	//xmlData.append("          나. 위  치 : " + STRU_ADDR_ADDR +" ("+AREA+"㎡ /"+ NUM_FL2 +") \n");
       	xmlData.append("          4. 용도 및 규모 : "+ OFFICE_TYPE +" ("+AREA+"㎡ /"+ NUM_FL2 +") \n"); 
       	       	
      	//xmlData.append("          5. 시공업체명 : " + PLANER_NAME + " / "+ PLANER_NM +"\n");
       	
   	 if (KJFUtil.isEmpty(PLANER_NAME)){
		xmlData.append("          5. 시공업체명 : " + PLANER_NM + "\n");
 	}
 	else {
 		xmlData.append("          5. 시공업체명 : " + PLANER_NAME+ "/" + PLANER_NM + "\n");
 	}
       	...
       	xmlData.append("          6. 검사결과 : " + SUV_APPL + "\n");
    	
       	//xmlData.append("  붙     임 : 검토 결과 보고서 1부.  끝.\n");
       	
    	if (!"비대상".equals(SUV_APPL)){
    		if (!"N".equals(ApplGugun)){
     			if ("cbji".equals(pm.getScSIGUNGU_CODE())){
        			xmlData.append("\n  붙임  설계도검토 결과 보고서 1부.  끝.\n");
        			}else{
        			xmlData.append("  붙     임 : 설계도검토 결과 보고서 1부.  끝.\n");
        			}
    	...
    	if (!"비대상".equals(SUV_APPL)){    		
    		xmlData.append("	<ATTACHMENTS>\n");
    		xmlData.append("		<ATTACHMENT filename=\"" + send_plan_file_name2 + ".html\">설계도검토 결과 보고서 1부.  끝.</ATTACHMENT>\n");
    		xmlData.append("	</ATTACHMENTS>\n");
    	}
    ...
    private String setAttachPlanFileInvest(UbParam pm) throws Exception{
        ...
        content.append("	<head>\n");
        content.append("		<meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\">\n");
        content.append("		<title>설계도 검토 결과 보고서 1부</title>\n");
        content.append("	</head>\n");
        content.append("	<body leftmargin=\"0\" topmargin=\"0\" marginwidth=\"0\" marginheight=\"0\">\n");
    ...
    public boolean sendEbmsPlanFileInvest(ActionForm form, String filePath, String destFolder)throws Exception{
		...
		if(sendFile(pm, filePath + "header.inf", destFolder, "header.inf")
			...
			flag = true;
			KJFLog.log("EBMS 파일 전송 완료4");
		}else{
			flag = false;
			KJFLog.log("EBMS 파일 전송 실패4");
		}
    	return flag;
    }  
...
//    	String dirPath = "D:\\work\\attache\\iccsm\\usebefore\\_ebms_dir\\";
    ...
    private String setExchangeFile(UbParam pm)throws Exception{
    	...
    	if ("jnwd".equals(pm.getScSIGUNGU_CODE())){
    	xmlData.append("			<TITLE>정보통신공사 사용전검사 결과보고(민원 제" + REG_NUM + "호)</TITLE>\n");
    	}else{
    	...
    	if ("jnwd".equals(pm.getScSIGUNGU_CODE())){
    		xmlData.append("        1. 민원 제"+REG_NUM+"(" + RECV_DT + ")호와 관련입니다.\n\n");
    		xmlData.append("        2. 정보통신공사 사용전검사 신청건에 대하여 정보통신공사업법 제 36조 및 동법 시행령 제 36조 규정에 의하여 검사한 결과를 보고합니다.\n\n");
    		xmlData.append("                              ▣  검사내역 ▣\n\n");
    	}
    	else {
    	...
    	if ("bsgs".equals(pm.getScSIGUNGU_CODE())){
    		if ("변경".equals(PLAN_ENT_CHG_YN)){
    			int f_index = PLAN_ENT_CHG_YN_CONT.indexOf(":");
    			if (f_index > 0){    				
    				xmlData.append("          - 설계 및 시공업체 : " + PLAN_ENT_CHG_YN_CONT.substring(f_index + 1, PLAN_ENT_CHG_YN_CONT.length())+ "/" + OPE_NAME + "\n");	
    			} else {
    				xmlData.append("          - 설계 및 시공업체 : " + PLAN_ENT_CHG_YN_CONT+ "/" + OPE_NAME + "\n");
    			}
    		} 
    	...
    	//else if ("jnmp".equals(pm.getScSIGUNGU_CODE())){
    	 if (KJFUtil.isEmpty(BEF_PLANER_NAME)){
			xmlData.append("          - 설계 및 시공업체 : " + OPE_NAME + "\n");
    	}
    	else {
    		xmlData.append("          - 설계 및 시공업체 : " + BEF_PLANER_NAME+ "/" + OPE_NAME + "\n");
    	}
		...
		if (!"N".equals(ApplGugun)){
			if ("jnwd".equals(pm.getScSIGUNGU_CODE())){
    		xmlData.append("\n붙      임  : 1. 정보통신공사 사용전검사  결과 보고서 1부  \n");
    		xmlData.append("              2. 정보통신공사 사용전검사  검사 결과 1부  \n");
    		xmlData.append("              3. 정보통신공사 사용전검사  검사 점검사진  1부.  끝.\n");
			}else if ("cbji".equals(pm.getScSIGUNGU_CODE())){
			xmlData.append("\n  붙임  정보통신공사 사용전검사 결과 보고서 1부.  끝.\n");
			}else{
			xmlData.append("붙      임 : 정보통신공사 사용전검사 결과 보고서 1부.  끝.\n");
			}
    	}
...
//    	xmlData.append("		</ADMINISTRATIVE_DB>\n");
    ...
    public boolean makeEbmsFileInvest(ActionForm form, String filePath)throws Exception{
    	...
    	String headerData 	= setHeaderFile_other(pm,"_Invest");	//header.inf 파일 내용
    	String exchangeData = setExchangeFileInvest(pm,"_Invest");	//exchange.xml 파일 내용
    ...
    private String setExchangeFileInvest(UbParam pm, String num)throws Exception{
    	...
    	if ("jnwd".equals(pm.getScSIGUNGU_CODE())){
    		xmlData.append("        1. 귀사의 무궁한 발전을 기원합니다.\n\n");
    		xmlData.append("        2. 귀하께서 신청하신 "+INSP_SPOT_DETAILADDR+"건축물에 대한 정보통신공사 사용전검사와\n");
    		xmlData.append("관련하여 정보통신공사업 법 제36조 및 동법 시행령 제36조 규정에 의하여 검사한 결과 적합하여\n");
    		xmlData.append("붙임과 같이 정보통신공사 사용전 검사 필증을 교부합니다.\n\n");
    		xmlData.append("                              ▣  교부내역 ▣\n");
    	}
    	else if ("gguw".equals(pm.getScSIGUNGU_CODE()))
    	{
        	xmlData.append("        1. 평소 정보통신 업무에 적극 협조하여 주심을 감사 드립니다.\n");
        	xmlData.append("        2. 귀하께서 신청하신 건축물(접수번호-"+REG_NUM+" : " + RECV_DT + ")에 대하여 정보통신공사업법 제36조 및 같은 법 시행령 제36조 규정에 의하여 검사한 결과 적합하여 붙임과 같이 정보통신공사 사용전검사 필증을 교부합니다.\n");
        	//xmlData.append("           제36조 및 같은 법 시행령 제36조 규정에 의하여 검사한 결과 적합하여 \n");
        	//xmlData.append("           붙임과 같이 정보통신공사 사용전 검사 필증을 교부합니다.\n");    	
        	xmlData.append("        ▣ 교부내역\n");
    	}
    	else {
        	xmlData.append("        1. 평소 정보통신 업무에 적극 협조하여 주심을 감사 드립니다.\n");
        	xmlData.append("        2. 귀하께서 신청하신 아래 건축물(접수번호-"+REG_NUM+" : " + RECV_DT + ")에 대하여 정보통신공사업법 제36조 및 같은 법 시행령 제36조 규정에 의하여 검사한 결과 적합하여 붙임과 같이 정보통신공사 사용전 검사 필증을 교부합니다.\n");
        	//xmlData.append("           제36조 및 같은 법 시행령 제36조 규정에 의하여 검사한 결과 적합하여 \n");
        	//xmlData.append("           붙임과 같이 정보통신공사 사용전 검사 필증을 교부합니다.\n");    	
        	xmlData.append("        ▣ 교부내역\n");
    	}
		...
		if (!"N".equals(ApplGugun)){
 			if ("cbji".equals(pm.getScSIGUNGU_CODE())){
    			xmlData.append("\n  붙임  정보통신공사 사용전검사 필증  1부.  끝.\n");
    			}
 			else if("jnwd".equals(pm.getScSIGUNGU_CODE())){
 				xmlData.append("붙  임 : 정보통신공사 사용전검사 필증  1부.  끝.\n");
 			}
 			else if("gguw".equals(pm.getScSIGUNGU_CODE())){
 				xmlData.append("\n 붙     임 : 정보통신공사 사용전검사 필증  1부.  끝.\n");
 			}
 			else{
    			xmlData.append("붙      임 : 정보통신공사 사용전검사 필증  1부.  끝.\n");
    			}
    ...
    public boolean makeEbmsFile_NAP_NO(ActionForm form, String filePath)throws Exception{
    	...
    	String headerData 	= setHeaderFile_other(pm,"_NAP_NO");	//header.inf 파일 내용
    	String exchangeData = setExchangeFile_NAP_NO(pm,"_NAP_NO");	//exchange.xml 파일 내용
...
//    	xmlData.append("있어, 보완을 통보하오니 조치 후 재검사를 신청하시기 바랍니다.\n");   
    	xmlData.append("        1. 평소 정보통신 업무에 적극 협조하여 주심을 감사 드립니다.\n");
    	xmlData.append("        2. 귀하께서 신청하신 아래 건축물(민원접수번호-"+REG_NUM+" : "+RECV_DT+")에 대하여 정보통신공사업법 제36조  및 같은 법 시행령 제26조의 규정에 의하여 검사한 결과 관련 기술에 부적합하게 시공된 부분이 있어 통보하오니 재시공후 재검사를 신청하시기 바랍니다.\n");
    	//xmlData.append("통신공사업법 제36조  및 같은 법 시행령 제26조의 규정에 의하여 검사한 결과 관련 기술에\n");
...
//    		NUM_FL = NUM_FL+", ";
    ...
    public boolean makeEbmsFile_NAP_SIDO_NO(ActionForm form, String filePath)throws Exception{
    	...
    	String headerData 	= setHeaderFile_other(pm,"_NAP_SIDO");	//header.inf 파일 내용
    	String exchangeData = setExchangeFile_NAP_SIDO_NO(pm,"_NAP_SIDO");	//exchange.xml 파일 내용
    ...
    private String setExchangeFile_NAP_SIDO_NO(UbParam pm, String num)throws Exception{
    	...
    	if ("bsgs".equals(pm.getScSIGUNGU_CODE())){
    		...
    		xmlData.append("        민원봉사과-"+REG_NUM+" "+RECV_DT+"호로 접수된 정보통신공사 사용전 검사 신청건과\n");
    	}
