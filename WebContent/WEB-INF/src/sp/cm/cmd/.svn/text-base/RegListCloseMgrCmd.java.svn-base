...
public class RegListCloseMgrCmd implements KJFCommand {
	
    private String next;	
    private DatasetList  dl = new DatasetList();
    private VariableList vl = new VariableList();
    
    public RegListCloseMgrCmd() {
    }

    public RegListCloseMgrCmd(String next_url) {
    	next = next_url;
    }
...
//        sQuery.append(" FROM PT_R_COMPANY_MASTER PT_M ");
       ...
       // sQuery.append(" FROM (PT_R_COMPANY_MASTER PT_M LEFT JOIN PT_M_MASTER PT_MM ON PT_M.TMP_WRT_NUM = PT_MM.TMP_WRT_NUM)");
        ...
        sQuery.append(" AND PT_M.C_COM_DT IS NOT NULL ");
        //sQuery.append(" AND PT_M.C_COM_DT IS NOT NULL AND TRANS_UNION_PROC IS NOT NULL AND TRANS_UNION_PARENT IS NOT NULL ");
        
        //sWhere.append(" WHERE 1 = 1 ");
        if (!KJFUtil.isEmpty(scDATE1) || !KJFUtil.isEmpty(scDATE2))
            sWhere.append(" AND(PT_M.C_COM_DT" + " BETWEEN '" + scDATE1 + "' AND '" + scDATE2 + "')  ");
