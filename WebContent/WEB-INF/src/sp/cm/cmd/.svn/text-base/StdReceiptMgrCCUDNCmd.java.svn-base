...
public class StdReceiptMgrCCUDNCmd implements KJFCommand {
    ...
    public StdReceiptMgrCCUDNCmd() {
    }


    public StdReceiptMgrCCUDNCmd(String next_url) {
    	next = next_url;
    }
    ...
    private void deleteExe(Dataset ds, int arg_row)throws Exception{
    	...
    	else
    	{
        	...
        	deleteQuery = " DELETE FROM PT_R_BASIC_STATEMENT WHERE RECV_NUM = '" + sKey + "'";
        	deleteQuery += " AND SIDO_CODE = '" + sSIDO_CODE + "'";
        	rDAO = new ReportDAO();
        	rDAO.execute(deleteQuery);
        	
        	deleteQuery = " DELETE FROM PT_R_MOT_STE WHERE RECV_NUM = '" + sKey + "'";
