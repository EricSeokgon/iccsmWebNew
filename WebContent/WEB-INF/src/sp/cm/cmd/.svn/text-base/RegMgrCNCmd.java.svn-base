...
public class RegMgrCNCmd<PT_R_DOC_NUM> implements KJFCommand {
    ...
    public RegMgrCNCmd(String next_url) {
    	next = next_url;
    }
    ...
    public String execute(HttpServletRequest request, ActionForm form) throws Exception {
        ...
        if (sworkMODE.equalsIgnoreCase("RECV_NUM"))
        {
	        ...
	        sWhere.append(" WHERE 1 = 1 ");
	        sWhere.append(" AND AA.TMP_WRT_NUM = '" + scRECV_NUM + "'");
        }
     ...
     // 대표자가 타업체 재직기술자 등록되면 안됨2
        else if (sworkMODE.equalsIgnoreCase("REFER_DUPLICATE_CHECK2"))
        {
        	...
        	String scSSN = vl.getValueAsString("scSSN");
		        ...
		        sWhere.append(" WHERE 1 = 1 ");
		        sWhere.append(" AND R.RET_YMD IS NULL ");
		        sWhere.append(" AND (R.ENGINEER_SSN1 || R.ENGINEER_SSN2) IN (" + scSSN + ")");
	        	sWhere.append(" AND C.TMP_WRT_NUM <> '" + scTMP_WRT_NUM + "'");
        ...
        else if (sworkMODE.equalsIgnoreCase("ENGINEER_DUPLICATE_CHECK"))
        {
        	...
        	for(int i = 0; i < dsPT_R_ENGINEER_CHANGE_TEMP.getRowCount(); i++ ) {
        		String scSSN = dsPT_R_ENGINEER_CHANGE_TEMP.getColumn( i, "ENGINEER_SSN1" ) + "" + dsPT_R_ENGINEER_CHANGE_TEMP.getColumn( i, "ENGINEER_SSN2" );
        		scSSN_IN += "'" + scSSN + "', ";
        	}
        	if( scSSN_IN.length() > 0 ) {
        		scSSN_IN = scSSN_IN.substring( 0, scSSN_IN.length() - 2 );
