...
public class TransMgrCNCmd<PT_R_DOC_NUM> implements KJFCommand {
    ...
    public String execute(HttpServletRequest request, ActionForm form) throws Exception {
    	...
    	String scTMP_WRT_NUM = vl.getValueAsString("scTMP_WRT_NUM");// 양도자 등록번호  양도자가 등록번호가 없을시에는 양수자 등록번호 값 을 받음
        ...
        else if((sworkMODE.equalsIgnoreCase("GET_CORP"))) //양도양수 접수관리-공사업체 조회
        {
 ...
 */
        ...
        else if((sworkMODE.equalsIgnoreCase("Engineer_List"))) //양도양수 접수관리-공사업체 조회
        {
        ...
        else if(sworkMODE.equalsIgnoreCase("GETDEF")) //양도양수 접수관리 상세보기 (흠결)
        {
	       	sQuery.delete(0, sQuery.length());
	       	sWhere.delete(0, sWhere.length());
        ...
        else if(sworkMODE.equalsIgnoreCase("DEFI_POPUP")) //양도양수 접수관리 상세보기
        {
        	//Deficit Codes Table
	    	sQuery.append(" SELECT * ");
	    	sQuery.append(" FROM PT_R_DEFICIT_CODE ");
        ...
        else if(sworkMODE.equalsIgnoreCase("TransView")) //양도양수 접수관리 상세보기
        {
...
//	        sQuery.append(" ,REP_MOBILE, LICTAX, HOSCRE, COM_NUM, ASSI_CHGBRE_SEQ, TRAN_CHGBRE_SEQ ");
	        ...
	        sQuery.append(" TRAN_FOREIGN_YN,TRAN_REGIST_YMD,decode(gubun,null,1,'본청',1,2,2,1) gubun,decode(COMMANA_CLASS,null,1,1,1,2,2,3,3) COMMANA_CLASS,decode(FOREIGN_YN,null,'N','N','N','Y','Y') FOREIGN_YN");
