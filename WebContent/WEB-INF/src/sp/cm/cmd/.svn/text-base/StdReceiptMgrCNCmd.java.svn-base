...
public class StdReceiptMgrCNCmd implements KJFCommand {
	
    private String next;	
    private DatasetList  dl = new DatasetList();
    private VariableList vl = new VariableList();
    
    public StdReceiptMgrCNCmd() {
    }

    public StdReceiptMgrCNCmd(String next_url) {
    	next = next_url;
    }
    ...
    public String execute(HttpServletRequest request, ActionForm form) throws Exception {
        ...
        if (workMODE.equalsIgnoreCase("GET_CORP"))
        {
        	...
        	sQuery.append(" SELECT NAME, TMP_WRT_NUM, MANA_NUM, REP_NM_KOR, ADDR_TEL_NUM, WRT_DT, ");
        ...
        else if (workMODE.equalsIgnoreCase("CHECK2"))
        {
        ...
        {
        	//Master Table
        	sQuery.append(" SELECT * FROM PT_R_BASIC_STATEMENT ");
        	sWhere.append(" WHERE	1=1	\n");
        	sWhere.append("  AND	RECV_NUM = '" + scRECV_NUM + "' ");
        	sWhere.append("  AND	SIDO_CODE = '" + scSIDO_CODE + "' ");
	        ...
	        sQuery.append(" (SELECT DEFI_CODE, CONT FROM PT_R_DEFICIT_CODE WHERE DEFI_GROUP = 'CLODEF') ");
        ...
        /*
         if (workMODE.equalsIgnoreCase("TRUE"))
        {
        ...
        else if((scCHECK.equalsIgnoreCase("FALSE")))
        {
	        ...
	        //Deficit Table
	        sQuery.append(" SELECT PDEF.*, PDEF_CODE.CONT AS DEFI_CONT FROM PT_R_DEFICIT PDEF LEFT JOIN ");
	        sQuery.append(" (SELECT DEFI_CODE, CONT FROM PT_R_DEFICIT_CODE WHERE DEFI_GROUP = 'TRNMGR') ");
	        sQuery.append(" PDEF_CODE  ON PDEF.DEFI_CODE = PDEF_CODE.DEFI_CODE ");
	        sWhere.append(" WHERE 1 = 1 ");
	        sWhere.append(" AND PDEF.RECV_NUM = '" + scRECV_NUM + "' ");
        ...
        else
        {
        	...
        	sQuery.append(" SELECT * ");
        	sQuery.append(" FROM PT_R_DEFICIT_CODE ");
        	//sQuery.append(" WHERE DEFICIT_GROUP = 'REGMGR_RECEIPT' ");
