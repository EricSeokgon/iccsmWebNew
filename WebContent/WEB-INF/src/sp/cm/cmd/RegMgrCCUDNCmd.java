...
public class RegMgrCCUDNCmd implements KJFCommand {
    ...
    public RegMgrCCUDNCmd() {
    }


    public RegMgrCCUDNCmd(String next_url) {
    	next = next_url;
    }
    ...
    public String execute(HttpServletRequest request, ActionForm form) throws Exception {   	    	
    	...
    	String EMPL_YMD = vl.getValueAsString("EMPL_YMD");
    	...
    	if (scMODE.equalsIgnoreCase("ALL"))
    	{
    		...
    		ds = pReq.getDatasetList().getDataset("dsPT_R_COM_NAME_CHANGE");
    		if (ds != null) dsProc(ds);
    		
    		ds = pReq.getDatasetList().getDataset("dsPT_R_ENGINEER_CHANGE");
    		if (ds != null) dsProc(ds);
    		
    		ds = pReq.getDatasetList().getDataset("dsPT_R_ADDRESS_CHANGE");
    		if (ds != null) dsProc(ds);
    		
    		ds = pReq.getDatasetList().getDataset("dsPT_R_COMPANY_DIAGNOSTIC");
    		if (ds != null) dsProc(ds);
    		
    		ds = pReq.getDatasetList().getDataset("dsPT_R_REPRESENT_CHANGE");
    		if (ds != null) dsProc(ds);
    		
    		ds = pReq.getDatasetList().getDataset("dsPT_R_REGMGR_COM_DEFICIT");
    		if (ds != null) dsProc(ds);
    		
    		ds = pReq.getDatasetList().getDataset("dsPT_R_REGMGR_ENG_DEFICIT");
    		if (ds != null) dsProc(ds);
    		
    		ds = pReq.getDatasetList().getDataset("dsPT_R_REGMGR_DIA_DEFICIT");
    		if (ds != null) dsProc(ds);
    		
    		ds = pReq.getDatasetList().getDataset("dsPT_R_REGMGR_OFFICE_DEFICIT");
    		if (ds != null) dsProc(ds);
    ...
    private void insertExe(Dataset ds, int arg_row)throws Exception{
    	...
    	{
    		String temp = "1";
    		temp = getMaxChangeSeq(ds.getColumnAsString(0, "TMP_WRT_NUM"), "PT_R_COMPANY_MASTER", "TMP_WRT_NUM");
    	...
    	{	
    		...
    		PT_R_ENGINEER_CHANGE_TEMPEnt.setSEQ(
    				getMaxChangeSeq(ds.getColumnAsString(arg_row, "TMP_WRT_NUM"), "PT_R_ENGINEER_CHANGE_TEMP", "SEQ"));
    		
    		PT_R_ENGINEER_CHANGE_TEMPEnt.setENG_PROC("R");
    	...
    	else if(ds.getId().equalsIgnoreCase("dsPT_R_ENGINEER_CHANGE"))
    	{
    		...
    		//2010.01.14 데이터베이스 필드명과 동일하게 변경하였음.
    		//PT_R_ENGINEER_CHANGEEnt.setTMP_FIELD(KJFMi.dsGet(ds, arg_row, "TMP_GRADE"));
    		//PT_R_ENGINEER_CHANGEEnt.setENGINEER_NM( KJFMi.dsGet(ds, arg_row, "NM_KOR") );
    ...
    private void deleteExe(Dataset ds, int arg_row)throws Exception{
    	...
    	//String sKey = vl.getValueAsString("scTMP_WRT_NUM");
    	System.out.println("KEY5 ============= " + sKey);
    	...
    	else if(ds.getId().equalsIgnoreCase("dsPT_R_ENGINEER_CHANGE_TEMP"))
    	{
    		...
    		System.out.println("KEY6 ============= " + sKey);
    		//String sSEQ = ds.getDeleteColumn(arg_row,"SEQ").getString();
    		String sSSN1 = ds.getDeleteColumn(arg_row,"ENGINEER_SSN1").getString();
    		String sSSN2 = ds.getDeleteColumn(arg_row,"ENGINEER_SSN2").getString();
    		System.out.println("KEY 1 " + sSSN1);
    		System.out.println("KEY 2 " + sSSN2);
    		//String sChgSeq = ds.getDeleteColumn(arg_row,"CHGBRE_SEQ").getString();
    		if(sSSN1 != null||sSSN2 != null){
    			        		
    			System.out.println("KEY 3 " + sSSN1);
        		System.out.println("KEY 4 " + sSSN2);
    		...
    		sQuery.append(" WHERE TMP_WRT_NUM = '" + sKey + "' ");
    		//sQuery.append(" AND SEQ = '" + sSEQ + "' ");
    		sQuery.append(" AND ENGINEER_SSN1 is null ");
    		sQuery.append(" AND ENGINEER_SSN2 is null ");
    		//sQuery.append(" AND CHGBRE_SEQ = '" + sChgSeq + "' ");
        	rDAO = new ReportDAO();
        	rDAO.execute(sQuery.toString());
    		}
    	...
    	{
    		//deleteQuery = " DELETE FROM PT_R_COM_NAME_CHANGE WHERE TMP_WRT_NUM = '" + sKey + "'";
        	//rDAO = new ReportDAO();
        	//rDAO.execute(deleteQuery);
        	...
        	//deleteQuery = " DELETE FROM PT_R_ADDRESS_CHANGE WHERE TMP_WRT_NUM = '" + sKey + "'";
        	//rDAO = new ReportDAO();
        	//rDAO.execute(deleteQuery);
        	
        	//deleteQuery = " DELETE FROM PT_R_COMPANY_DIAGNOSTIC WHERE TMP_WRT_NUM = '" + sKey + "'";
        	//rDAO = new ReportDAO();
        	//rDAO.execute(deleteQuery);
        	
        	//deleteQuery = " DELETE FROM PT_R_REPRESENT_CHANGE WHERE TMP_WRT_NUM = '" + sKey + "'";
        	//rDAO = new ReportDAO();
        	//rDAO.execute(deleteQuery);
        	...
        	deleteQuery = " DELETE FROM PT_R_SUBSIDIARY WHERE TMP_WRT_NUM = '" + sKey + "'";
        	rDAO = new ReportDAO();
        	rDAO.execute(deleteQuery);
        	...
        	deleteQuery = " DELETE FROM PT_R_COMPANY_MASTER WHERE TMP_WRT_NUM = '" + sKey + "'";
        	rDAO = new ReportDAO();
        	rDAO.execute(deleteQuery);
    	}    	
    }
