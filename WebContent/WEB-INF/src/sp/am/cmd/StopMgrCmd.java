...
public class StopMgrCmd implements KJFCommand {
	    ...
	    public StopMgrCmd(String next_url) {
	    	next = next_url;
	    }
...
/*
	        sQuery.append(" SELECT '0' AS CHECKER, PT_C.NAME, PT_S.WRT_NUM, PT_S.TMP_WRT_NUM, ");
	        sQuery.append(" PT_M.MOT_STE, PT_M.WRT_DT, PT_M.D_PER_CODE, PT_M.DISPO_DT, ");
	        ...
	        sQuery.append(" INNER JOIN PT_M_SUSPENSION PT_S ON PT_M.WRT_NUM = PT_S.WRT_NUM	");
...
*/
	        ...
	        sWhere.append(" WHERE 	1 = 1 ");	 
	        sWhere.append(" AND PT_M.DISPO_CONT = 'M00002' ");	// 영업정지 코드
	        sWhere.append(" AND		(PT_M.WRT_DT BETWEEN '" + scDATE1 + "' AND '" + scDATE2 + "') ");
	        if(scWRT_NUM.length() > 0)
	        	sWhere.append(" AND     PT_S.WRT_NUM = '" + scWRT_NUM + "' ");
	        if(scCOM_NAME.length() > 0)
