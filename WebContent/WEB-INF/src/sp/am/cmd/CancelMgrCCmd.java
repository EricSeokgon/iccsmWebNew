...
public class CancelMgrCCmd implements KJFCommand {
	 
		private String next;	
	    private DatasetList  dl = new DatasetList();
	    private VariableList vl = new VariableList();
	    
	    public CancelMgrCCmd() {
	    }

	    public CancelMgrCCmd(String next_url) {
	    	next = next_url;
	    }
	    ...
	    public String execute(HttpServletRequest request, ActionForm form) throws Exception {
	        ...
	        {
	        	sQuery.append(" SELECT  * FROM PT_M_REGIST_CANCEL ");
		        sWhere.append(" WHERE 	1 = 1 ");	 
		        ...
		        sQuery.append(" SELECT  * FROM PT_M_AUDI_REPORT ");
		        rEntity    = rDAO.select(sQuery.toString() + sWhere.toString());    	
		        KJFMi.ReEnt2Ds(dl,rEntity,"output3");
		        sQuery.delete(0, sQuery.length());
		        
		        sQuery.append(" SELECT  * FROM PT_M_AUDI_DICTION ");
		        rEntity    = rDAO.select(sQuery.toString() + sWhere.toString());    	
		        KJFMi.ReEnt2Ds(dl,rEntity,"output4");
		        sQuery.delete(0, sQuery.length());
		        
		        sQuery.append(" SELECT  * FROM PT_M_AUDI_OPINION ");
		        rEntity    = rDAO.select(sQuery.toString() + sWhere.toString());    	
...
//		        sQuery.append(" ) BB ON AA.NOTE_CLASS_CODE = BB.CODE ");
		        ...
		        sQuery.append(" SELECT  '0' AS CHECKER,  " +
		        		" AA.SEQ,AA.NOTE_CLASS_CODE,AA.SEND_DT,AA.S_PER,AA.RECV_YN,AA.REMARK,AA.INS_DT," +
		        		" AA.UPD_DT,AA.WRT_ID,AA.TMP_WRT_NUM,AA.WRT_NUM,AA.NOTE_METHOD_CLASS,AA.RECV_LOC,AA.GUBUN, " +
		        		" BB.CODE_NAME AS TMP_GUBUN, CC.CODE_NAME AS NOTE_NAME  ");
		        sQuery.append(" ,  DD.CODE_NAME AS TMP_NOTE_METHOD, EE.CODE_NAME AS TMP_RECV_YN  ");
		        sQuery.append(" FROM PT_M_AUDI_BEF_REPORT AA ");
		        sQuery.append(" LEFT JOIN ( SELECT P_CODE, CODE_NAME, CODE FROM PT_COM_CODE  ");
		        sQuery.append(" WHERE P_CODE = 'PTMGUB' ");
		        sQuery.append(" ) BB ON AA.GUBUN = BB.CODE ");
		        sQuery.append(" LEFT JOIN ( SELECT P_CODE, CODE_NAME, CODE FROM PT_COM_CODE  ");
