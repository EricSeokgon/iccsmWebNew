...
import sp.dao.PT_M_WARNINGDAO;
import sp.dao.PT_M_WARNINGEntity;
...
public class WarnMgrCUDCmd implements KJFCommand {
    ...
    private PT_M_WARNINGDAO PT_M_WARNINGdao = new PT_M_WARNINGDAO();
    private PT_M_WARNINGEntity PT_M_WARNINGEnt = new PT_M_WARNINGEntity();
    
    private VariableList vl = new VariableList();
    
    public WarnMgrCUDCmd() {
    }


    public WarnMgrCUDCmd(String next_url) {
    	next = next_url;
    }
    ...
    public String execute(HttpServletRequest request, ActionForm form) throws Exception {   	    	
    	...
    	if (scworkMODE.equalsIgnoreCase("INSERT_SAVE"))
    	{
	    	...
	    	ds = pReq.getDatasetList().getDataset("dsPT_M_WARNING");
	    	if(ds != null) dsProc(ds);
    	}
    ...
    private void insertExe(Dataset ds, int arg_row)throws Exception{
    	...
    	else if(ds.getId().equalsIgnoreCase("dsPT_M_WARNING"))
    	{
    		PT_M_WARNINGEnt.setDsOnlyString(ds, arg_row);// 
           	KJFLog.debug(PT_M_WARNINGEnt.toString());
           	PT_M_WARNINGEnt.setINS_DT(KJFDate.getCurDatetime());
           	PT_M_WARNINGdao.insert(PT_M_WARNINGEnt);
    	}
    ...
    private void updateExe(Dataset ds, int arg_row)throws Exception{
    	...
    	else if(ds.getId().equalsIgnoreCase("dsPT_M_WARNING"))
    	{
    		PT_M_WARNINGEnt.setDsOnlyString(ds, arg_row);// 
           	KJFLog.debug(PT_M_WARNINGEnt.toString());
           	PT_M_WARNINGEnt.setUPD_DT(KJFDate.getCurDatetime());
           	PT_M_WARNINGdao.update(PT_M_WARNINGEnt);
    	}
    	else if(ds.getId().equalsIgnoreCase("dsPT_M_MEMO"))
    	{
    		PT_M_MEMOEnt.setDsOnlyString(ds, arg_row);// 
