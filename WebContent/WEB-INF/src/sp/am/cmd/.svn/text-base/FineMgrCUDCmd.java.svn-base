...
import sp.dao.PT_M_PROSECUTIONDAO;
import sp.dao.PT_M_PROSECUTIONEntity;
...
public class FineMgrCUDCmd implements KJFCommand {
    ...
    private PT_M_PROSECUTIONDAO PT_M_PROSECUTIONdao = new PT_M_PROSECUTIONDAO();
    private PT_M_PROSECUTIONEntity PT_M_PROSECUTIONEnt = new PT_M_PROSECUTIONEntity();
    
    private VariableList vl = new VariableList();
    
    public FineMgrCUDCmd() {
    }


    public FineMgrCUDCmd(String next_url) {
    	next = next_url;
    }
    ...
    public String execute(HttpServletRequest request, ActionForm form) throws Exception {   	    	
    	...
    	if (scworkMODE.equalsIgnoreCase("INSERT_SAVE"))
    	{
	    	...
	    	ds = pReq.getDatasetList().getDataset("dsPT_M_PROSECUTION");
	    	if(ds != null) dsProc(ds);
    	}
    ...
    private void insertExe(Dataset ds, int arg_row)throws Exception{
    	...
    	else if(ds.getId().equalsIgnoreCase("dsPT_M_PROSECUTION"))
    	{
    		PT_M_PROSECUTIONEnt.setDsOnlyString(ds, arg_row);// 
           	KJFLog.debug(PT_M_PROSECUTIONEnt.toString());
           	PT_M_PROSECUTIONEnt.setINS_DT(KJFDate.getCurDatetime());
           	PT_M_PROSECUTIONdao.insert(PT_M_PROSECUTIONEnt);
    	}
    ...
    private void updateExe(Dataset ds, int arg_row)throws Exception{
    	...
    	else if(ds.getId().equalsIgnoreCase("dsPT_M_PROSECUTION"))
    	{
    		PT_M_PROSECUTIONEnt.setDsOnlyString(ds, arg_row);// 
           	KJFLog.debug(PT_M_PROSECUTIONEnt.toString());
           	PT_M_PROSECUTIONEnt.setUPD_DT(KJFDate.getCurDatetime());
           	PT_M_PROSECUTIONdao.update(PT_M_PROSECUTIONEnt);
    	}
    	else if(ds.getId().equalsIgnoreCase("dsPT_M_MEMO"))
    	{
    		PT_M_MEMOEnt.setDsOnlyString(ds, arg_row);// 
    ...
    private void deleteExe(Dataset ds, int arg_row)throws Exception{
    	...
    	else
    	{
	    	...
	    	sQuery.append(" DELETE FROM dsPT_M_PROSECUTION ");
	    	rDAO.execute(sQuery.toString() + sWhere.toString());
	    	sQuery.delete(0, sQuery.length());
    	}
    }
