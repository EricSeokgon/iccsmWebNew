...
<%
	...
	String post_num_temp = masterEntity.getValue(0,"ADDR_POST_NUM");
	String post_num_temp2 = masterEntity.getValue(0,"ADDR_POST_OLD");
	if(post_num_temp.length() >= 6) {
		post_num_temp = post_num_temp.substring(0,3) + "-" + post_num_temp.substring(3,6);
	} 
	if(post_num_temp2.length() >= 6) {
		post_num_temp2 = post_num_temp2.substring(0,3) + "-" + post_num_temp2.substring(3,6);
	}
	System.out.println(post_num_temp);
	String addr_post_num = KJFUtil.print("("+post_num_temp+")",""); //우편번호
	...
	String addr_post_old = KJFUtil.print("("+post_num_temp2+")",""); //예전 우편번호
	String addr1_old = masterEntity.getValue(0,"ADDR1_OLD"); //예전 주소
	String addr2_old = masterEntity.getValue(0,"ADDR2_OLD"); //예전 상세주소
...
<body>
<form name="searchfrm" action="../reg/RegAction.do?cmd=RegMgrView" method="post">
			...
			<a name="contents"></a>
	   			<div id="contents">
	   				...
	   				<h4><font size="4">공사업자 세부정보 (현 등록업자 정보)</font></h4>
	   				<br/>
					...
					<table  class="t7">
						<tr>
							<th >등록번호<br/>(사업자번호)</th>
							<td class="letter0" align="center"><%=coi_wrt_num %><br/><%=mana_num %></td>
							<th>상호<br/>(법인번호)</th>
							<td  colspan="2" class="letter0" align="center"><%=name %><br/><%=cum_num %></td>
							<th>등록일자</th>
							<td class="letter0" align="center"><%=buss_ymd %></td>
						</tr>
						<tr>
							<th>대표자</th>
							<td class="letter0" align="center"><%=rep_nm_kor %></td>
							<th>주민번호</th>
							<td colspan="2" class="letter0" align="center"><%=rep_ssn1 %>-<%=rep_ssn2 %></td>
							<th>사무실면적</th> 
							<td class="letter0" align="center"><%=office_area %> ㎡</td>
						</tr>
						<tr>
							<th rowspan="2">영업소재지</th>
							<th>도로명</th>
	   					    <td colspan="3" ><%=addr_post_num %>&nbsp;<%=addr_addr %>&nbsp;<%=addr_detail_addr %></td>
							<th>전화번호</th>
	    					<td class="letter0" align="center"><%=addr_tel_num %></td>
						</tr>
						<tr>
						    <th>지번</th>
							<td  colspan="3" ><%=addr_post_old %>&nbsp;<%=addr1_old %>&nbsp;<%=addr2_old %></td>
							<th>팩스번호</th>
							<td class="letter0" align="center"><%=addr_fax_num %></td>
						</tr>
					...
					<%
						...
						if(disposeEntity.getRowCnt() > 0){ 
							for(int i=0; i<disposeEntity.getRowCnt(); i++) {
								...
								busisusp_start_dt = KJFUtil.print(disposeEntity.getValue(i, "BUSISUSP_START_DT"),"");
								busisusp_end_dt = KJFUtil.print(disposeEntity.getValue(i, "BUSISUSP_END_DT"),"");
					...
					<%
						...
						if(changeCnt > 0){ 
							...
							for(int i=startCnt; i<changeEntity.getRowCnt(); i++) {
								//if(i >= 3) break;
					%>			
						<tr>
