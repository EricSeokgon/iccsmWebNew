...
function PrintPage() {
	...
	myControl.prop_stringToSend = document.frm.PAY_IN_CAPITAL_AMT_K.value+"  "+"\r\n";
	myControl.ex_send_1_string();
	...
	if(document.frm.SITE_ADDR.value.length>60-k){
		myControl.prop_stringToSend = document.frm.SITE_ADDR.value.substring(0,60-k)+" "+"\r\n";
		myControl.ex_send_1_string();
		
		myControl.prop_horizontal_tab = 18;
		myControl.ex_set_horizontal_tab();
		
		myControl.prop_stringToSend = document.frm.SITE_ADDR.value.substring(60-k)+" "+"\r\n\r\n";
		myControl.ex_send_1_string();
	}else{
